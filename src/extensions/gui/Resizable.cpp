...
namespace doge::gui
{
    ...
    void Resizable::Initialize()
    {
        ...
        tl.on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f(-1, -1)); on_resized((start_size - act_size) * Vec2f(  1,   1) * GetAlign()); };
        t .on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f( 0, -1)); on_resized((start_size - act_size) * Vec2f(  0,   1) * GetAlign()); };
        tr.on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f( 1, -1)); on_resized((start_size - act_size) * Vec2f( -1,   1) * GetAlign()); };
        r .on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f( 1,  0)); on_resized((start_size - act_size) * Vec2f( -1,   0) * GetAlign()); };
        br.on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f( 1,  1)); on_resized((start_size - act_size) * Vec2f( -1,  -1) * GetAlign()); };
        b .on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f( 0,  1)); on_resized((start_size - act_size) * Vec2f(  0,  -1) * GetAlign()); };
        bl.on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f(-1,  1)); on_resized((start_size - act_size) * Vec2f(  1,  -1) * GetAlign()); };
        l .on_dragged_diff += [&](const Vec2f& diff){ auto act_size = SetSizeChecked(start_size + diff * Vec2f(-1,  0)); on_resized((start_size - act_size) * Vec2f(  1,   0) * GetAlign()); };
